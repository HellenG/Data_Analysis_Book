...
circle <- function(n = 1, r = 1, h = 0, v = 0, half = FALSE, up = TRUE, plot = TRUE, xlab = "x", ylab = "y", circ.border = "chocolate", lwd = 1, lty = 1, ...) {
   ...
   # Circle equation
   circ <- function(x, r, h, v) sqrt(r^2 - (x-h)^2) + v
   circ2 <- function(x, r, h, v) sqrt(r^2 - (x-h)^2) - v
   ...
   if(half) {
      ...
      y <- lapply(1:n, function(i) c(circ(x[[i]], r[i], h[i], v[i]), -circ2(rev(x[[i]]), r[i], h[i], v[i])))
...
Initial topic we will cover is "colouring" circles. Here we want to re-introduce circles as regards formulation of their equation and applying definite intergration to color different sections. 


## Formulating circle equation

Circles are graphs whose points have an equal distance from a central point. Distance between center and any point on a cicle is referred to as **radius**.
...
We want to figure out x and y coordinates of points on this graph begining with point (x, y) as shown below. 
...
We begin with unions which require colouring of both circles.
...
A circle has four different intervals, to shade it requires using rectangles with height set to specific interval. For example left quandrant will have it's rectangles with height of it's left point. These rectangles are made to very small values so that they leave no space. 
...
Disjoint are basically two circles that do not touch each other, therefore a no brainer to plot. 
...
Complement of a set are all elements in another but not that set. Like complement of A here are all elements in universal set that are not in A. We show this with a transparent circle for A and coloured background to indicate universal set. 

Initially began with backgound colour by using one rectangle then coloured circle.  
...
Difference are all elements in one set not in another set. Coloring this region required full circle to be coloured and difference (center region) to be removed by coloring it differently. 
